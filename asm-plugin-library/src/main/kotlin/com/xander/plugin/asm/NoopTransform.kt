...
open class NoopTransform(var project: Project) : Transform() {
  ...
  override fun transform(transformInvocation: TransformInvocation) {
    ...
    // OutputProvider 管理输出路径，如果消费型输入为空，你会发现OutputProvider == null
    ...
    for (input in inputs) {
      for (jarInput in input.jarInputs) {
        ...
        println("transform jar:${jarInput.file.absolutePath} dest:${dest.absolutePath}" )
      ...
      for (directoryInput in input.directoryInputs) {
        ...
        println("transform directory:${directoryInput.file.absolutePath} dest:${dest.absolutePath}")
        //将修改过的字节码copy到dest，就可以实现编译期间干预字节码的目的了
